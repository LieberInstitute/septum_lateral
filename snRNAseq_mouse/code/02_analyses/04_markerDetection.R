...
sapply(cellSubtype.idx, function(x){quantile(sce.ls$sum[x])})     ambig.glial_A ambig.glial_B Aqp4.Rbpms_A Aqp4.Rbpms_B Astro_A Astro_B
...
for(i in smaller.set){
  png(here("plots","snRNA-seq","LS_t_pairwise_top40markers-", i, "_logExprs.png"), height=950, width=1200)
  print(
...
# 20-40 markers
for(i in left.set){
  png(here("plots","snRNA-seq","LS_t_pairwise_top40markers-", i, "_logExprs.png"), height=1900, width=1200)
  print(
    plotExpressionCustom(sce = sce.hold,
                         features = genes.top40.t[[i]], 
                         features_name = i,
...
save(markers.ls.t.pw, markers.ls.wilcox.block, markers.ls.t.1vAll, medianNon0.ls,
     file="rdas/revision/markers-stats_LS-n4_findMarkers.rda")
...
for(i in names(genes.top40.t)){
  png(here("plots","snRNA-seq","LS_t_1vALL_top40markers-",i,"_logExprs.png"), height=1900, width=1200)
  print(
    plotExpressionCustom(sce = sce.hold,
                         features = genes.top40.t[[i]], 
                         features_name = i,
